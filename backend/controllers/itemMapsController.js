...
exports.allItemMaps = async (req, res) => {
  ...
  const itemMaps2 = await req.app.db('item_maps').select()
  console.log(itemMaps2)
...
exports.newItemMap = async (req, res) => {
  ...
  const companyMapping = await req.app.db('item_maps').where({ ic_id: req.params.id, map_ic_id: req.body.map_ic_id });
